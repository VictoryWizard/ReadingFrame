...
export function HomeHero({ latestSlug }: { latestSlug: string }) {
  const { tagline, mission } = getSiteConfig();
  ...
  return (
    <section className="relative min-h-[min(85vh,720px)] overflow-hidden">
      ...
      />
      <div
        className="absolute inset-0 bg-[var(--rf-hero-overlay)]"
        aria-hidden="true"
      />
      <div className="rf-container relative flex min-h-[min(85vh,720px)] flex-col justify-center py-20 text-white">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <Link href="/posts/" className="rf-btn rf-btn-ghost border-white/30 text-white hover:bg-white/10">
            Browse archive
          </Link>
        </div>
      </div>
